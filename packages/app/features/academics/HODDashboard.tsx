...
export const HODDashboard = () => {
  const { data: syllabus } = useQuery({
    queryKey: ['syllabus'],
    queryFn: async () => {
      const res = await fetch('/api/academics/syllabus', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } });
      return res.json();
    }
  });
  ...
  const columns = [
    ...
    { header: "Status", accessor: (row: any) => (
      <SovereignBadge status={row.status === 'ON_TRACK' ? 'success' : 'warning'}>{row.status.replace('_', ' ')}</SovereignBadge>
    )}
  ];
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <SovereignTable data={syllabus || []} columns={columns} />
    </div>
  );
};
